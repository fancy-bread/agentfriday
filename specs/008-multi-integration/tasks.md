...
- [x] T009 [P] [US2] Update `src/cli/index.ts` — remove `.option('--integration <tool>', ...)` from the `init` command; remove `integration` from the options object passed to `runInit()`
...
- [x] T011 [P] Run `npm run typecheck` — fix any TypeScript errors across `src/integration/`, `src/cli/configure.ts`, `src/cli/init.ts`, `src/cli/status.ts`, `src/cli/index.ts`
- [x] T012 [P] Run `npm test` — all suites pass including new `tests/integration/configure.test.ts`
